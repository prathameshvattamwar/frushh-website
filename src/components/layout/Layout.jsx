    ...
    <div className="min-h-screen bg-gray-50 pb-20 md:pb-0">
        ...
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-1">
              <NavLink to="/" className={({ isActive }) => `px-4 py-2 rounded-lg font-medium ${isActive ? 'bg-green-100 text-green-700' : 'text-gray-600'}`}>Home</NavLink>
              <NavLink to="/menu" className={({ isActive }) => `px-4 py-2 rounded-lg font-medium ${isActive ? 'bg-green-100 text-green-700' : 'text-gray-600'}`}>Menu</NavLink>
              <NavLink to="/orders" className={({ isActive }) => `px-4 py-2 rounded-lg font-medium ${isActive ? 'bg-green-100 text-green-700' : 'text-gray-600'}`}>Orders</NavLink>
              <NavLink to="/rewards" className={({ isActive }) => `px-4 py-2 rounded-lg font-medium ${isActive ? 'bg-green-100 text-green-700' : 'text-gray-600'}`}>Rewards</NavLink>
              <NavLink to="/refer" className={({ isActive }) => `px-4 py-2 rounded-lg font-medium ${isActive ? 'bg-pink-100 text-pink-700' : 'text-gray-600'}`}>Refer</NavLink>
            </nav>

            <div className="flex items-center gap-2">
              <NavLink to="/cart" className="hidden md:flex relative items-center gap-2 px-4 py-2 bg-gray-100 rounded-lg">
              ...
              {user ? (
                <NavLink to="/profile" className="flex items-center gap-2 px-3 py-2 bg-gray-100 rounded-lg">
                ...
                <NavLink to="/login" className="px-4 py-2 bg-green-500 text-white rounded-lg">Login</NavLink>
              )}
              {user?.is_admin && (
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t z-50">
        <div className="flex justify-around py-2">
          ...
          <NavLink to="/menu" className={`flex flex-col items-center py-1 ${location.pathname.startsWith('/menu') ? 'text-green-600' : 'text-gray-500'}`}>
            <i className="fa-solid fa-utensils text-xl"></i>
            <span className="text-xs mt-1">Menu</span>
          </NavLink>
          <NavLink to="/cart" className={`flex flex-col items-center py-1 relative ${location.pathname === '/cart' ? 'text-green-600' : 'text-gray-500'}`}>
            <i className="fa-solid fa-cart-shopping text-xl"></i>
            {cartCount > 0 && <span className="absolute top-0 right-2 w-4 h-4 bg-green-500 text-white text-xs rounded-full flex items-center justify-center">{cartCount}</span>}
