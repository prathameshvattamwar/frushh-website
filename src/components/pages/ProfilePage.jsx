    ...
    async function fetchData() {
      try {
        ...
        const { data: gymsData } = await supabase
          ...
          .eq('is_active', true)
          .order('display_order')
        if (isMounted && gymsData) setGyms(gymsData)

        const { data: addressData } = await supabase
          .from('user_addresses')
          .select('*')
          .eq('user_id', user.id)
        if (isMounted && addressData) setAddresses(addressData)
        ...
        const { data: pointsData } = await supabase
          ...
          .eq('user_id', user.id)
          .maybeSingle()
        if (isMounted && pointsData) setLoyaltyPoints(pointsData)
        ...
        const { data: referralData } = await supabase
          ...
          .eq('user_id', user.id)
          .maybeSingle()
        if (isMounted && referralData) setReferralCode(referralData.code)
      ...
      <div className="max-w-2xl mx-auto px-4 py-4">
          ...
          <div className="space-y-4">
            ...
            {referralCode && (
              <div className="bg-gradient-to-r from-pink-500 to-purple-600 rounded-2xl p-4 text-white flex items-center justify-between">
                ...
                <Link
                  to="/refer"
                  className="px-4 py-2 bg-white text-purple-600 font-medium rounded-lg"
                >
                  Share
                </Link>
              </div>
            ...
            <button
              ...
              className="w-full py-3 bg-red-50 text-red-600 font-medium rounded-xl hover:bg-red-100"
            >
              Logout
            </button>
          </div>
        ...
        {activeTab === 'addresses' && (
          <div className="space-y-3">
            {addresses.length === 0 ? (
              ...
              addresses.map(addr => (
                <div
                  key={addr.id}
                  className="bg-white rounded-2xl p-4 flex items-start justify-between"
                >
                  <div>
          ...
          <div className="space-y-4">
            ...
            <Link
              to="/rewards"
              className="block w-full py-3 bg-green-500 text-white font-bold rounded-xl text-center"
            >
              View Rewards
            </Link>
          </div>
